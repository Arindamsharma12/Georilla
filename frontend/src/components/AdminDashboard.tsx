    ...
    <div className="flex bg-[#0f0f10] min-h-screen w-full">
      ...
      <nav className="w-full h-16 bg-gradient-to-r from-gray-900 to-gray-800 flex items-center justify-between px-6 fixed top-0 left-20 z-50">
        ...
        <div className="flex space-x-4 text-sm font-medium items-center overflow-x-auto whitespace-nowrap">
          ...
          <a href="signin.html" className="text-white hover:text-gray-300">
            Sign in
          </a>
          <a
            href="/admin"
            className="text-white hover:text-yellow-400 font-bold border border-yellow-400 px-3 py-1 rounded transition"
          >
            Admin
          </a>
        </div>
          ...
          {stats.map((stat, idx) =>
            stat.link ? (
              <a
                key={idx}
                href={stat.link}
                className="card text-center hover:ring hover:ring-blue-500 transition-all"
              >
                <p>{stat.label}</p>
              ...
              <div key={idx} className="card col-span-1 text-center">
                ...
                {stat.button && (
                  <button className="mt-4 px-4 py-1 bg-blue-600 text-xs rounded-lg">
                    {stat.button}
                  </button>
                )}
                {stat.sub && (
        ...
        <div className="card text-sm">
          ...
          <div className="flex flex-col md:flex-row justify-between items-center mb-4 gap-4">
            <input
              type="text"
              placeholder="Search..."
              className="p-2 text-xs bg-gray-800 rounded w-full md:w-1/3"
            />
            <input
              type="date"
              className="p-2 text-xs bg-gray-800 rounded w-full md:w-auto"
            />
            <button className="px-4 py-1 text-xs bg-blue-600 rounded">
              Advanced Filters
            </button>
          </div>
